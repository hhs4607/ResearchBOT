...
import { ArrowLeft, ExternalLink, Activity, Network, CheckCircle2, BookmarkMinus, Sparkles, Calendar, BrainCircuit, BookOpen } from "lucide-react";
      ...
      <header className="sticky top-0 z-10 bg-background/95 backdrop-blur border-b px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-2">
            <Badge variant="outline" className={`font-mono text-sm ${getScoreColor(paper.ai_relevance_score)}`}>
              Score: {(paper.ai_relevance_score * 100).toFixed(0)}
            </Badge>
            {(paper.is_included === true || paper.is_included === 1) && (
              <Badge className="bg-green-600 hover:bg-green-600 gap-1"><CheckCircle2 className="w-3.5 h-3.5" /> Included</Badge>
            )}
            {(paper.is_included === false || paper.is_included === 0) && (
              <Badge variant="destructive" className="gap-1"><BookmarkMinus className="w-3.5 h-3.5" /> Excluded</Badge>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <CopyProjectDialog paperId={paper.id} currentProjectId={paper.project_id} />
          <Button variant="default" size="sm" className="gap-2" disabled={paper.is_included === true || paper.is_included === 1}>
            <CheckCircle2 className="h-4 w-4" /> Include
          </Button>
          <Button variant="secondary" size="sm" className="gap-2 text-destructive hover:bg-destructive/10" disabled={paper.is_included === false || paper.is_included === 0}>
        ...
        <div className="mx-auto max-w-5xl space-y-8">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-muted-foreground">
              ...
              <span className="flex items-center gap-1"><Calendar className="h-4 w-4" /> {paper.year}</span>
            ...
            <div className="lg:col-span-2 space-y-8">
              <section>
                <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
                  <BookOpen className="h-5 w-5 text-muted-foreground" /> Abstract
                </h3>
              ...
              <section>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold flex items-center gap-2">
                    <BrainCircuit className="h-5 w-5 text-primary" /> AI Extraction
                  </h3>
              ...
              <div className="rounded-xl border bg-card p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-2xl font-bold">{paper.cited_by_count}</p>
                  </div>
                  <div className="space-y-1">
                    <span className="text-sm text-muted-foreground">References</span>
                    <p className="text-2xl font-bold">{paper.references?.length || 0}</p>
                  </div>
                </div>
              </div>
                    ...
                    <div className="max-h-[400px] overflow-y-auto p-4 space-y-4">
                      {paper.references?.map((ref, i) => (
                        <div key={i} className="text-sm">
                          <p className="font-medium hover:underline cursor-pointer line-clamp-2">{ref.title}</p>
                  ...
                  <TabsContent value="citations" className="p-0 m-0">
                    <div className="max-h-[400px] overflow-y-auto p-4 space-y-4">
                      {paper.cited_by?.map((cite, i) => (
                        <div key={i} className="text-sm">
                          <p className="font-medium hover:underline cursor-pointer line-clamp-2">{cite.title}</p>
