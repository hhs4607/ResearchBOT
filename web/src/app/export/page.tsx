...
import { Download, RefreshCw, AlertCircle, FolderOpen } from "lucide-react";
...
export default function ExportPage() {
  ...
  const handleExportCsv = () => {
    ...
    window.location.href = `${base}/api/projects/${projectId}/export/csv`;
  };
    ...
    return (
      <div className="flex h-64 items-center justify-center">
        <div className="text-center text-muted-foreground">
          <FolderOpen className="w-12 h-12 mx-auto mb-4 opacity-50" aria-hidden="true" />
  ...
  return (
    <div className="flex h-full flex-col">
      <header className="sticky top-0 z-10 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b px-6 py-4">
        <h1 className="text-2xl font-bold tracking-tight">Export & Integrations</h1>
        <p className="text-sm text-muted-foreground mt-1">
          {currentProject ? currentProject.name : "Project"} — download your curated papers and AI extractions.
        </p>
      </header>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="rounded-xl border bg-card text-card-foreground shadow-sm hover:shadow-md transition-shadow p-6">
              ...
              <div className="mt-4 pt-4 border-t">
                ...
                >
                  <RefreshCw className={`h-4 w-4 ${syncZoteroMutation.isPending ? "animate-spin" : ""}`} aria-hidden="true" />
